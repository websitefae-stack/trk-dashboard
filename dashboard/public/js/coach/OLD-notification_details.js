(function () {
  ...
  async function saveStatus() {
    const docname = el("notificationDocname")?.value || "";
    const status = el("notificationStatus")?.value || "";

    if (!docname || !status) return;

    const response = await fetch("/api/method/dashboard.api.coach.notifications.update_notification_status", {
      method: "POST",
