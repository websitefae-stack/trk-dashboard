...
def _get_client_rows_for_coach_me(coach_context):
    if not frappe.db.exists("DocType", "Client"):
        return []
...
def _get_calendar_for_options(coach_context):
    options = [
        {
...
def _get_selected_calendar_for(selected_calendar_for, coach_context):
    ...
    return COACH_ME_VALUE, options


def _get_client_options_for_calendar(selected_calendar_for, coach_context):
    rows = (
        _get_client_rows_for_coach_me(coach_context)
        if selected_calendar_for == COACH_ME_VALUE
        else _get_client_rows_for_worker(selected_calendar_for)
    )
...
def _get_coach_events(range_start_date, range_end_date, selected_calendar_for, coach_context):
...
def get_calendar_bootstrap(week_start=None, view=None, date=None, selected_worker=None, selected_calendar_for=None):
    ...
    selected_calendar_for, calendar_for_options = _get_selected_calendar_for(selected_calendar_for, coach_context)

    return {
        "events": _get_coach_events(range_start_date, range_end_date, selected_calendar_for, coach_context),
        "clients": _get_client_options_for_calendar(selected_calendar_for, coach_context),
        ...
        "current_worker_label": "Me" if selected_calendar_for == COACH_ME_VALUE else _get_session_worker_label(selected_calendar_for),
...
def get_event_details(event=None):
    ...
    return {
        ...
        "status": raw_status,
        "ui_status": ui_status,
        "worker_label": _get_session_worker_label(client_row.get("session_worker")) if client_row.get("session_worker") else "Me",
