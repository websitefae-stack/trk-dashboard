...
# "schoolname" all resolve the same way, since this is filled in by hand
# in a spreadsheet rather than generated by the system.
...
def _normalise_import_header(header):
...
def import_schools_from_csv():
    """
    Bulk-imports schools (and their contacts) from an uploaded CSV - one
    row per contact, with School Name/Website repeated on every row for
    that school (or left blank on the 2nd+ row for the same school -
    either works, rows are grouped by School Name either way).

    Desk's own Data Import tool needs System Manager access to run
    against School/School Contact (see _managed_permissions() in
    create_school_pipeline_doctypes.py), which the franchisor account
    doesn't have - this is a dashboard-native alternative that doesn't
    need it, going through _ensure_franchisor() instead like every other
    endpoint in this file.

    A School Name that already exists (case-insensitive match) has its
    new contacts added to it rather than creating a duplicate School -
    already-present emails on that school are skipped, not duplicated,
    so the same file can be safely re-uploaded (e.g. after fixing a few
    rows) without doubling everything up.
    """
    _ensure_franchisor()
    ...
    if not reader.fieldnames:
        frappe.throw(_("That file doesn't look like a CSV - couldn't find a header row."))

    header_map = {}
    for header in reader.fieldnames:
        canonical = IMPORT_COLUMN_ALIASES.get(_normalise_import_header(header))
        if canonical:
            header_map[header] = canonical

    if "school_name" not in header_map.values():
        frappe.throw(_("The file needs a \"School Name\" column."))

    # Existing schools, keyed by a case/whitespace-insensitive version of
    # their name, so "Oakwood Primary" and "oakwood primary " match the
    # same record instead of creating a second one.
    existing_schools = {}
    for row in frappe.get_all(SCHOOL_DOCTYPE, fields=["name", "school_name"], ignore_permissions=True):
        key = (row.school_name or "").strip().lower()
        if key:
    ...
    for line_number, raw_row in enumerate(reader, start=2):  # header is line 1
        ...
        if not school_name and groups:
            # Blank School Name on a continuation row - belongs to
            # whichever school the previous row in the file was for.
            school_name = groups[-1][0]

        if not school_name:
            row_errors.append(f"Row {line_number}: no School Name given.")
            continue
        ...
        if not contact_name and not email:
            ...
            # keep the school, just nothing to add to its contacts list.
            continue

        if not contact_name or not email:
            row_errors.append(f"Row {line_number}: contact needs both a name and an email - skipped.")
            continue
    ...
    for school_name, website, area, contact_rows in groups:
        ...
        if existing_name:
            doc = frappe.get_doc(SCHOOL_DOCTYPE, existing_name)
            is_new_school = False
        else:
            doc = frappe.new_doc(SCHOOL_DOCTYPE)
            doc.school_name = school_name
        ...
        for contact_row in contact_rows:
            email_key = contact_row["email"].strip().lower()
