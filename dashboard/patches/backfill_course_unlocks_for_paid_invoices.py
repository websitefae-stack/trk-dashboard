...
course access, portal login, and the access email. A no-op if that
field isn't set on anything yet - the franchisor can re-run it any time
afterwards via the whitelisted function instead, since a patch only
ever runs once per site.
"""
